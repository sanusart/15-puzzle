...
export default function puzzleReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case ACTION.SYNC_STORAGE: {
      let updates = {};
      if (state.hallOfFame.length === 0 && window.localStorage.getItem('hallOfFame')) {
        updates.hallOfFame = JSON.parse(window.localStorage.getItem('hallOfFame'));
      }
      return { ...state, ...updates };
    }
    ...
    case ACTION.ADD_TO_WALL_OF_FAME: {
      ...
      window.localStorage.setItem('hallOfFame', JSON.stringify(updates.hallOfFame));
      updates.hallOfFame = JSON.parse(window.localStorage.getItem('hallOfFame'));
      return { ...state, ...updates };
    }
